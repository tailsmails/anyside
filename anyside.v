...
mut:
	...
	seq_out         u8
	byte_pool       []u8
	expected_seq_in u8 = 1
	reorder_buffer  map[u8]FrameData
	adapter_proc    os.Process
}
	...
	lock state {
		for state.byte_pool.len >= header_len + crc_len {
			...
			if crc32.sum(frame_part) == received_crc {
				is_valid = true
				final_frame = frame_part.clone()
			} else {
				if recovered := try_recover(frame_part, received_crc) {
					if cfg.verbose { println('[!] Recovered Frame') }
					is_valid = true
					final_frame = recovered.clone()
				}
			}

			if is_valid {
				cmd := final_frame[2]
				conn_id := final_frame[3]
				seq := final_frame[4]
				payload := final_frame[header_len..].clone()
				raw_frame := FrameData{cmd: cmd, conn_id: conn_id, seq: seq, payload: payload}
				diff := u8(seq - state.expected_seq_in)
				if diff == 0 {
					valid_frames << raw_frame
					state.expected_seq_in++
					for {
						next_seq := state.expected_seq_in
						if next_frame := state.reorder_buffer[next_seq] {
							valid_frames << next_frame
							state.reorder_buffer.delete(next_seq)
							state.expected_seq_in++
						} else {
							break
						}
					...
					if seq !in state.reorder_buffer {
						state.reorder_buffer[seq] = raw_frame
					}
				} else {
					if cfg.verbose { println('[!] Duplicate/Late packet skipped: seq=$seq') }
				}
...
fn execute_tx(cfg Config, data []u8, shared state AppState) {
	...
	lock state {
		state.adapter_proc.stdin_write('TX ${b64_str}\n')
	}
}
...
fn execute_rx(cfg Config, shared state AppState) []u8 {
	mut all_bytes := []u8{}
	lock state {
		state.adapter_proc.stdin_write('RX\n')
		
		mut output_accum := ''
		for {
			chunk := state.adapter_proc.stdout_read()
			if chunk != '' {
				output_accum += chunk
				if output_accum.contains('__END_BATCH__') {
					break
				}
			}
		...
		lines := output_accum.split('\n')
		for line in lines {
			trimmed := line.trim_space()
			if trimmed == '__END_BATCH__' {
				break
			}
			if trimmed != '' {
...
fn main() {
	...
	cmd_parts := cfg.adapter_cmd.split(' ')
	exe := cmd_parts[0]
	mut args := cmd_parts[1..].clone()
	args << mode
